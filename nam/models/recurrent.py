...
class SkippyLSTM(BaseNet):
    ...
    def __init__(
        ...
        **lstm_kwargs,
        ...
        if "batch_first" in lstm_kwargs:
            raise ValueError("batch_first cannot be set.")
        self._input_size = input_size
        self._core = _SkippyLSTM(
            self._input_size, hidden_size, num_layers=num_layers, **lstm_kwargs
        )
        self._head = nn.Linear(self._core.output_size, 1)
        self._train_burn_in = train_burn_in
        self._train_truncate = train_truncate
    ...
    def _forward(self, x):
        ...
        if x.ndim==2:
            x = x[:, :, None]
        last_hidden_state = self._core.initial_state(x)
        if not self.training or self._train_truncate is None:
            output_features = self._core(x, last_hidden_state)[0]
        else:
            output_features_list = []

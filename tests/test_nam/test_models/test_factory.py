...
def _registry_backup():
    ...
    Usage:
        with _registry_backup():
            ...
            _factory.register("TestModel", some_constructor)
            # ... test logic ...
    """
    # Get the original registry state
    original_registry = _copy.deepcopy(_factory._model_net_init_registry)
    ...
    try:
        ...
        _factory._model_net_init_registry.clear()
        _factory._model_net_init_registry.update(original_registry)
...
class TestRegistry:
    ...
    def test_default_registry_contents(self, model_name, config, expected_type):
        ...
        model = _factory.init(model_name, kwargs={"config": config})
    ...
    def test_register_successful(self):
        ...
        with _registry_backup():
            ...
            _factory.register("MockModel", mock_constructor)

            # Test that we can initialize it
            model = _factory.init("MockModel")
    ...
    def test_register_duplicate_with_overwrite(self):
        ...
        with _registry_backup():
            ...
            _factory.register("Linear", mock_constructor, overwrite=True)

            # Test that the new constructor is used
            model = _factory.init("Linear")
            assert isinstance(model, _linear.Linear)
    ...
    def test_init_import_based_fallback_missing_module(self):
        ...
        with _pytest.raises(
            ...
            _factory.init("nonexistent.module.Factory")
    ...
    def test_init_import_based_fallback_missing_factory(self):
        ...
        with _pytest.raises(
            ...
            _factory.init("nam.models.linear.NonExistentFactory")
    ...
    def test_init_with_invalid_args(self):
        ...
        with _pytest.raises(Exception):  # The specific exception depends on the model
            _factory.init("Linear", kwargs={"invalid_param": "invalid_value"})
    ...
    def test_multiple_registrations(self):
        ...
        with _registry_backup():
            ...
            # Register both
            _factory.register("TestModel1", mock_constructor1)
            _factory.register("TestModel2", mock_constructor2)

            # Test both
            model1 = _factory.init("TestModel1")
            model2 = _factory.init("TestModel2")
    ...
    def test_registry_with_complex_models(self):
        ...
        seq_model = _factory.init("Sequential", kwargs={"config": seq_config})
    ...
    def test_registry_backup_handles_exceptions(self):
        ...
        try:
            with _registry_backup():
                ...
                _factory.register("TestExceptionModel", test_constructor)

                # Verify it's registered
                assert "TestExceptionModel" in _factory._model_net_init_registry
        ...
        assert _factory._model_net_init_registry == original_registry
        assert "TestExceptionModel" not in _factory._model_net_init_registry

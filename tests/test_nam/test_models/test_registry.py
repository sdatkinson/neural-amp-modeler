...
class TestRegistry:
    ...
    @_pytest.mark.parametrize(
        ...
        [
            (
                "Linear",
                {"receptive_field": 2, "sample_rate": 44100},
                _linear.Linear
            ),
            (
                "ConvNet",
            ...
            (
                "LSTM",
                {"hidden_size": 10, "sample_rate": 44100},
                _recurrent.LSTM
            ),
            (
                "WaveNet",
            ...
            (
                ...
                {
                    "models": [
                        {"name": "Linear", "config": {"receptive_field": 2, "sample_rate": 44100}},
                        {"name": "Linear", "config": {"receptive_field": 3, "sample_rate": 44100}},
                    ]
                },
    ...
    def test_register_duplicate_without_overwrite(self):
        ...
        with _pytest.raises(KeyError, match="A constructor for net name 'Linear' is already registered!"):
            _registry.register("Linear", _linear.Linear.init_from_config, overwrite=False)
    ...
    def test_init_with_args_and_kwargs(self):
        ...
        model1 = _registry.init("Linear", kwargs={"config": {"receptive_field": 2, "sample_rate": sample_rate}})
        assert isinstance(model1, _linear.Linear)
        
        # Test with args only (empty kwargs)
        model2 = _registry.init("Linear", args=(), kwargs={"config": {"receptive_field": 2, "sample_rate": sample_rate}})
        assert isinstance(model2, _linear.Linear)
        
        # Test with both args and kwargs
        model3 = _registry.init("Linear", args=(), kwargs={"config": {"receptive_field": 2, "sample_rate": sample_rate}})
    ...
    def test_init_with_none_args_kwargs(self):
        ...
        model = _registry.init("Linear", args=None, kwargs={"config": {"receptive_field": 2, "sample_rate": sample_rate}})
        assert isinstance(model, _linear.Linear)
    ...
    def test_init_import_based_fallback(self):
        ...
        model = _registry.init("nam.models.linear.Linear", kwargs={"receptive_field": 2, "sample_rate": sample_rate})
        assert isinstance(model, _linear.Linear)

    def test_init_import_based_fallback_missing_module(self):
        """Test import-based initialization with missing module."""
        with _pytest.raises(KeyError, match="No importable module found for name 'nonexistent.module.Factory'"):
    ...
    def test_init_import_based_fallback_missing_factory(self):
        ...
        with _pytest.raises(KeyError, match="No factory found for name 'nam.models.linear.NonExistentFactory' within module 'nam.models.linear'"):
    ...
    def test_multiple_registrations(self):
        ...
        with _registry_backup():
            ...
            def mock_constructor2(*args, **kwargs):
                return _conv_net.ConvNet(channels=4, dilations=[1, 2], sample_rate=44100)
    ...
    def test_registry_with_complex_models(self):
        ...
        seq_config = {
            "models": [
                {"name": "Linear", "config": {"receptive_field": 2, "sample_rate": sample_rate}},
                {"name": "Linear", "config": {"receptive_field": 3, "sample_rate": sample_rate}},
            ]
        }
        ...
        conv_model = _registry.init("ConvNet", kwargs={
            "config": {"channels": 8, "dilations": [1, 2, 4, 8], "sample_rate": sample_rate}
        })
